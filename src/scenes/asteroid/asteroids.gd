...
func _ready():
	...
	for _i in range(asteroid_count):
		...
		asteroid.connect("asteroid_hit", Callable(self, "on_asteroid_hit"))
		add_child(asteroid)
...
func random_position_within_circle(spread_radius):
	while true:
		var position = Vector2(randf_range(-spread_radius, spread_radius), randf_range(-spread_radius, spread_radius))
